...
export class McpGatewayBoardService {
  ...
  async refresh(force = false) {
    ...
    try {
      ...
      // 容器列表低频节流刷新（默认轮询 8s，容器 15s 一次），避免每次 SSH 执行
      // docker ps 造成双倍开销，同时保证面板能看到容器最新状态（含已退出）。
      const containersPromise = Date.now() - this.lastContainersRefresh >= 15_000
        ? this._refreshContainers().then(() => {
            this.lastContainersRefresh = Date.now()
          })
        : Promise.resolve()
  ...
  static DOCKER_OPS = {
    ...
    rmi: { change: true, json: false, timeout: 60_000 },
  ...
  async _docker(action) {
    ...
    switch (op) {
      ...
      case 'rmi': {
        // targets = 镜像名/ID 列表（支持批量）；镜像名含 : / @，用 _safeImage 校验
        const imgs = targets.map((t) => McpGatewayBoardService._safeImage(String(t))).filter(Boolean)
        if (imgs.length === 0) throw new Error('镜像名非法')
        const flag = action.force ? '-f ' : ''
        cmd = `docker rmi ${flag}${imgs.map((i) => this._q(i)).join(' ')}`
        break
      }
      case 'pull': {
      ...
      case 'run': {
        ...
        // 保持运行：未提供命令时自动追加保活命令，避免容器主进程退出即停止（默认开启）
        if (action.keepAlive !== false && cmdTokens.length === 0) {
          parts.push('sh', this._q('-c'), this._q('tail -f /dev/null'))
        }
        cmd = parts.join(' ')
        break
      }
  ...
  // 必须监听 'error'：exe 不存在时 Node 会抛未处理 error 事件，直接拖垮整个 dsh 进程。
  async _gatewayStartLocal(profile, port) {
    const active = this._active()
    if (!active || !active.exec || typeof active.exec._gatewayExe !== 'function') {
      throw new Error('本机目标执行器未就绪')
    }
    ...
    try {
      await new Promise((resolve, reject) => {
        ...
        const onError = (err) => {
          finish(reject, new Error(`启动本机 gateway 失败: ${err && err.message ? err.message : err}（exe=${exe}）`))
        }
    ...
    child.unref?.()
    // 常驻进程后续若再报错，吞掉以免拖垮 dsh
    child.on('error', () => {
      /* ignore late spawn errors */
    })
    await this.refresh(true)
    return {
      ok: true,
