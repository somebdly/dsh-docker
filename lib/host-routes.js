...
// GET  /api/dsh-docker/state        -> 看板快照（profiles + servers + gateway + 精简 catalog）
// GET  /api/dsh-docker/catalog?q=   -> 搜索仓库 catalog servers（服务端过滤缓存）
// POST /api/dsh-docker/action       -> { kind, ... } 动作，返回快照/结果
// POST /api/dsh-docker/shell/open   -> { container, shell? } 打开交互终端，返回 { session }
// GET  /api/dsh-docker/shell/stream?session=xxx -> SSE 事件流（pty-out / exit）
// POST /api/dsh-docker/shell/input  -> { session, data } 写入终端输入
// POST /api/dsh-docker/shell/resize -> { session, cols, rows } 调整终端尺寸
// POST /api/dsh-docker/shell/close  -> { session } 关闭终端

const API_PREFIX = '/api/dsh-docker'
