  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        const state = {
          ...
          selectedImages: [], // 镜像批量删除勾选：镜像删除标识列表（repo:tag 或 ID）
        }
        const disposers = []
        ...
        // 镜像页不自动刷新：轮询/切回页面时若重建表格会打断用户勾选、滚动。
        // 镜像数据在切到镜像页时（cview-images / tab-containers）加载一次，用户可用「刷新」按钮强制更新。
        const isImagesView = () => state.tab === 'containers' && state.cview === 'images'
        ...
        const startPoll = () => {
          ...
          state.pollTimer = setInterval(() => { void pollTick() }, Math.max(3000, interval))
        }
        const stopPoll = () => {
        ...
        const toggle = (st) => {
          ...
          if (st.open) {
            ...
            // 兼容不响应 dsh-panel-activate 的兄弟面板（如 taskagent-board 只认 ssh）：
            // 它们打开时用自己的 active 属性 + CSS 隐藏中心列其它 view（包括本面板），
            // 且不因本面板的激活事件关闭自己，导致本面板 view 永远被藏。主动摘除其 active 标志。
            document.documentElement.removeAttribute('data-dsh-taskagent-board-active')
        ...
        const onVisibility = () => {
          if (state.open && document.visibilityState === 'visible' && !isImagesView()) void refresh()
        }
        ...
        function mcpHtml(snap, st) {
          ...
          const gatewayBar =
            `<div class="mcp-bar mcp-gateway-bar">` +
            `<span class="mcp-dot ${gwDot}"></span>` +
            `<span>Gateway ${gwRunning ? `运行中:${esc(String(gw.port || '?'))}` : '已停止'}</span>` +
            `<span class="mcp-meta">${esc(snap.gatewayProfile || gw.profile || '')}</span>` +
            (gwRunning
              ? `<button class="mcp-btn" data-act="gateway-restart">重启 Gateway</button>`
              : `<button class="mcp-btn" data-act="gateway-start">启动 Gateway</button>`) +
            `</div>`
          ...
          return gatewayBar + subTabs + content
        }
        ...
        function containersHtml(snap, st) {
          ...
          const viewSwitch =
            ...
            (cview === 'ps'
              ...
              : `<button class="mcp-btn" data-act="docker-pull">拉取镜像</button>` +
                ...
                `<button class="mcp-btn mcp-danger-ghost" data-act="docker-rmi" ${st.selectedImages.length === 0 ? 'disabled' : ''}>删除所选${st.selectedImages.length > 0 ? ` (${st.selectedImages.length})` : ''}</button>`) +
            `</div>`
          ...
          if (cview === 'images') {
            ...
            const selected = new Set(st.selectedImages || [])
            const rows = images.map((im) => {
              const size = im.Size || ''
              const tag = im.Tag || ''
              const repo = im.Repository || ''
              const fullName = tag ? `${repo}:${tag}` : repo
              // 删除标识：有 tag 用 repo:tag，悬空镜像（<none>）用 ID
              const delKey = (tag && tag !== '<none>') ? fullName : (im.ID || fullName)
              const checked = selected.has(delKey) ? ' checked' : ''
              return (
                `<tr>` +
                `<td><input type="checkbox" class="mcp-img-check" data-act="image-select" data-key="${esc(delKey)}"${checked}></td>` +
                ...
                `<button class="mcp-btn mcp-mini mcp-danger" data-act="docker-rmi-one" data-key="${esc(delKey)}" data-name="${esc(fullName)}">删除</button>` +
                `</td>` +
                `</tr>`
              )
            }).join('')
            const allSelected = images.length > 0 && images.every((im) => {
              const tag = im.Tag || ''
              const repo = im.Repository || ''
              const fullName = tag ? `${repo}:${tag}` : repo
              const delKey = (tag && tag !== '<none>') ? fullName : (im.ID || fullName)
              return selected.has(delKey)
            })
            return viewSwitch +
              `<div class="mcp-table-wrap"><table class="mcp-table">` +
              `<thead><tr>` +
              `<th><input type="checkbox" class="mcp-img-check-all" data-act="image-select-all"${allSelected ? ' checked' : ''}></th>` +
              `<th>镜像</th><th>标签</th><th>大小</th><th>ID</th><th>操作</th>` +
              `</tr></thead>` +
        ...
        async function doDockerRun(st, image) {
          ...
          const body =
            ...
            `<label class="mcp-field mcp-field-check"><input type="checkbox" id="mcp-run-keep" checked>` +
            `<span>保持运行（未填命令时自动追加 <code>tail -f /dev/null</code> 保活，防止容器启动后退出）</span></label>` +
        ...
        async function doDockerRunConfirm(st) {
          ...
          const keepAlive = document.getElementById('mcp-run-keep')?.checked ?? true
          const data = await doAction({ kind: 'docker', op: 'run', targets: [image], name, ports, env, volumes, cmd, keepAlive })
          if (data) {
            closeTopModal()
        ...
        async function handleAct(act, el, st, event) {
          switch (act) {
            ...
            case 'image-select': {
              ...
              const sel = Array.isArray(st.selectedImages) ? st.selectedImages.slice() : []
              const idx = sel.indexOf(key)
              if (checked && idx === -1) sel.push(key)
              if (!checked && idx !== -1) sel.splice(idx, 1)
              st.selectedImages = sel
              render(st)
              break
            }
            case 'image-select-all': {
              const checked = el.checked
              const snap = st.snapshot
              const images = (snap && Array.isArray(snap.images)) ? snap.images : []
              const keys = images.map((im) => {
                const tag = im.Tag || ''
                const repo = im.Repository || ''
                const fullName = tag ? `${repo}:${tag}` : repo
                return (tag && tag !== '<none>') ? fullName : (im.ID || fullName)
              })
              st.selectedImages = checked ? keys : []
              render(st)
              break
            }
            case 'docker-rmi': {
              const keys = Array.isArray(st.selectedImages) ? st.selectedImages.slice() : []
              ...
              for (const im of images) {
                ...
                const fullName = tag ? `${repo}:${tag}` : repo
                const delKey = (tag && tag !== '<none>') ? fullName : (im.ID || fullName)
                byKey[delKey] = { key: delKey, name: fullName, tag: im.Tag || '', id: im.ID || '' }
              }
              const picked = keys.map((k) => byKey[k]).filter(Boolean)
              const names = picked.map((p) => `<span class="mcp-mono">${esc(p.name)}</span>`).join('<br>')
              const body =
                `<p>确定要删除以下 <b class="mcp-text-danger">${picked.length}</b> 个镜像吗？</p>` +
                `<div class="mcp-rmi-list">${names || esc(keys.join('<br>'))}</div>` +
                `<p class="mcp-warn">⚠ 此操作不可撤销！被容器使用的镜像将无法删除（除非强制）。</p>` +
                `<div class="mcp-modal-actions">` +
                `<button class="mcp-btn" data-act="modal-close">取消</button>` +
                `<button class="mcp-btn mcp-danger" data-act="docker-rmi-confirm">确认删除</button>` +
                `</div>`
              showModal(modalHtml('批量删除镜像', body))
              break
            }
            case 'docker-rmi-one': {
              const key = el.dataset.key || ''
              ...
              const body =
                `<p>确定要删除镜像 <b class="mcp-text-danger">${esc(name)}</b> 吗？</p>` +
                `<p class="mcp-warn">⚠ 此操作不可撤销！被容器使用的镜像将无法删除（除非强制）。</p>` +
                `<div class="mcp-modal-actions">` +
                `<button class="mcp-btn" data-act="modal-close">取消</button>` +
                `<button class="mcp-btn mcp-danger" data-act="docker-rmi-confirm" data-key="${esc(key)}">确认删除</button>` +
                `</div>`
              showModal(modalHtml('删除镜像', body))
              break
            }
            case 'docker-rmi-confirm': {
              // 批量：取 st.selectedImages；单个：取 el.dataset.key
              let keys = (el.dataset.key !== undefined && el.dataset.key !== '')
                ? [el.dataset.key]
                : (Array.isArray(st.selectedImages) ? st.selectedImages.slice() : [])
              if (keys.length === 0) break
              const force = true // 与容器删除一致，默认强制（-f）
              const data = await doAction({ kind: 'docker', op: 'rmi', targets: keys, force })
              if (data) {
                closeTopModal()
                st.selectedImages = []
                showModal(modalHtml('删除完成', `<p class="mcp-dim">${esc(data.output || '(无输出)')}</p>` +
                  `<div class="mcp-modal-actions"><button class="mcp-btn" data-act="modal-close">关闭</button></div>`))
                void refresh()
              }
              break
        ...
        function openTargetEditor(tid, st) {
          ...
          const body =
            ...
            // 新建默认类型是 SSH：初始就显示 SSH 字段；打开后再 sync() 与下拉框对齐
    ...
    function injectStyle() {
      ...
      style.textContent = `
        ...
        .mcp-field-check{display:flex;flex-direction:row;align-items:center;gap:8px;cursor:pointer}
        .mcp-field-check input[type="checkbox"]{accent-color:#58a6ff;width:14px;height:14px;flex:none}
        .mcp-field-check code{background:rgba(88,166,255,0.12);border:1px solid rgba(88,166,255,0.3);border-radius:4px;padding:0 4px;font-size:12px}
        ...
        .mcp-rmi-list{max-height:180px;overflow:auto;background:var(--bg,#11151b);border:1px solid var(--border,#2a2e37);border-radius:6px;padding:8px 10px;margin:8px 0;font-size:12px}
        .mcp-table input[type="checkbox"]{accent-color:#f85149;width:14px;height:14px;cursor:pointer}
