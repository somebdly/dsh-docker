...
export class LocalExecService {
  ...
  // 查找顺序：docker.exe 同目录 → ~/.docker/cli-plugins → PATH 上的 docker-mcp
  _gatewayExe() {
    const candidates = []
    if (this.dockerPath) {
      const sibling = this.dockerPath.replace(/docker\.exe$/i, 'docker-mcp.exe')
      if (sibling && sibling !== this.dockerPath) candidates.push(sibling)
    }
    candidates.push(join(homedir(), '.docker', 'cli-plugins', 'docker-mcp.exe'))
